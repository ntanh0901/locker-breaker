// Baseline Algorithm Analysis Script - Original Version (Before Optimization)
// Tests the original Mastermind solver against all possible 4-digit codes (0000-9999)
...
// ORIGINAL ALGORITHM: Only minimax scoring (no entropy optimization)
function calculateGuessScore(guess, possibleSolutions) {
  ...
  // ORIGINAL: Always use worst-case scenario (largest group size) - NO ENTROPY
...
function getBestNextGuess(possibleSolutions, guessCount, debug = false) {
  ...
  // ORIGINAL: No information-maximizing guesses for small sets
  // Just add some basic strategic guesses for larger sets
  if (possibleSolutions.length > 100) {
    // Use deterministic strategic guesses
...
function runAnalysis(options = {}) {
  ...
  console.log("Starting BASELINE algorithm analysis...");
  console.log(
    `Testing ${maxCases} cases from ${startFrom} to ${startFrom + maxCases - 1}`
  );
  console.log("Using ORIGINAL minimax-only approach (no entropy optimization)");
  ...
  const report = {
    ...
    algorithm: "baseline_minimax_only",
    duration: duration,
    totalCases: maxCases,
  ...
  // Print summary
  console.log("\n" + "=".repeat(60));
  console.log("BASELINE ALGORITHM ANALYSIS COMPLETE");
  console.log("=".repeat(60));
  console.log(`Algorithm: ORIGINAL (minimax-only, no entropy optimization)`);
  ...
  const filename = `baseline-results-${startFrom}-${
...
function runQuickTest() {
  console.log("Running BASELINE quick test on first 100 cases...");
  return runAnalysis({ maxCases: 100, debug: false, logInterval: 25 });
}

function runMediumTest() {
  console.log("Running BASELINE medium test on first 1000 cases...");
  return runAnalysis({ maxCases: 1000, debug: false, logInterval: 100 });
}

function runFullTest() {
  console.log("Running BASELINE full test on all 10000 cases...");
...
if (require.main === module) {
  ...
  switch (testType) {
    ...
    default:
      console.log(
        "Usage: node baseline-algorithm-analysis.js [quick|medium|full|debug]"
      );
